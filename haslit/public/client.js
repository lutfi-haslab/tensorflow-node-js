...
var element_default = br;

// main.tsx
var App = () => {
  const [state, setState] = import_react.default.useState(false);
  const toggle = element_default.toggle({
    label: "Toggle Exampleee",
    onClick: () => setState(!state)
  });
  const elements = [
    toggle,
    state && element_default.write({ body: String(state) }),
    element_default.write({ body: "Hai" }),
    element_default.write({ body: "Hasi" }),
    element_default.write({ body: "Hai" }),
    element_default.write({ body: String(state) }),
    element_default.write({ body: "Hai" }),
    element_default.write({ body: "Hai" })
  ];
  return import_react.default.createElement("div", null, ...elements);
};

// haslit/client.tsx
var import_react2 = __toESM(require_react(), 1);
client.hydrateRoot(document.getElementById("root"), import_react2.default.createElement(App));
